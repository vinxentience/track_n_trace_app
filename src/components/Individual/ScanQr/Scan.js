    ...
    const handleScanFile = (result) => {
        if(result){
            db.collection('users').doc(result).get().then(doc => {
                if(doc.data().role === "Establishment"){
                    setEstablishment(doc.data().estabName)
                    setAddress(doc.data().estabAddress)
                    setDateTime(moment().format('MMMM Do YYYY, h:mm:ss a'))
                    setIsOpen(true)
                    setScanedId(result)
                } else { 
                    setIsOpen(false)
                    alert("This is not a registered establishment.")
                }
            })
        }
    }
            ...
            <Container maxW="xl" mt={100} centerContent>
                <Grid templateColumns={{ base: "repeat(1, 1fr)", md: "repeat(1, 1fr)", lg: "repeat(1, 1fr)"}} gap={0} w={{ base: "400px", md: "400px", lg: "400px" }}>
                    ...
                    <Box ml="3">
                        ...
                        <Text fontSize="sm">{getEstablishment}</Text>
                    </Box>
                    </Flex>
      ...
      >
        <AlertDialogOverlay>
          <AlertDialogContent>
            ...
            <AlertDialogBody>
              Are you going inside {getEstablishment} at {getAddress}? Please kindly confirm.
            </AlertDialogBody>
